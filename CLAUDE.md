...
- **Handed pre-merge checks are a merge gate.** When a review lane hands verification greps or commands alongside a PR, run them against the branch tip and record the result on the PR before marking it ready — ratification must never race a pending check (twice: PR #24 merge order, PR #31 verify-line drift; both harmless, both needed dated corrections).
- **Monitors get a fire-drill before they're trusted.** Any watcher/monitor that reports by exception must prove, before it's armed, that it can emit both its positive signal and a failure signal (run the parse pipeline green once; force the error path loud). Silence from an instrument is indistinguishable from a dead instrument — twice bitten (a `--json` flag, a missing `jq`). Non-vacuity applies to the instruments themselves; the same who-watches-the-watcher reasoning that gave the observer node its external pinger.
