...
class GcpAdapter(CloudAdapter):
    ...
    def fetch_metrics(self, provider: CloudProvider) -> dict[str, Any]:
        return {
            ...
            "note": (
                "Set credentials_ref (e.g. GCP_SERVICE_ACCOUNT_JSON) and deploy a "
                "Compute Engine Tess stack; metrics merge with local /health probes."
                if not provider.credentials_ref
                else "GCP credentials ref present; Monitoring pull stubbed until SDK wired."
            ),
        }
