...
**Graph checkpoints (W3) — recoverable-loss run scratch, doctrine-compatible.** The
product graph's flag-gated checkpointer (`GRAPH_CHECKPOINTING_ENABLED`, default off;
`app/graph/checkpoint.py`) stores per-run checkpoints in Redis under `graph:ckpt*` keys,
TTL-bounded (`GRAPH_CHECKPOINT_TTL_SECONDS`, default 3600 — sized from a measured L4 run:
19 checkpoints, max 77 KB single, ~839 KB whole-run). This does NOT reopen the
"Redis as durable authority" question: checkpoints are classified **recoverable-loss** —
a lost checkpoint means a run restarts; never corruption, never split authority — which
is exactly the caches + pub/sub class. Checkpoint threads are strictly per-run
(`thread_id = f"{session_id}:{panel_id}"`) and never outlive their turn;
`conversation:{session_id}` remains the sole cross-turn store. etcd was explicitly
rejected as a checkpoint backend: a 1.5 MiB request ceiling and a quorum write per
superstep are the wrong contract for run scratch that tolerates loss.
...
The bundle is also **failover-certified on its own topology** (restored by W1.5, the
offline-verifier topology re-sync): the verifier's split-brain step runs the harness
against the offline stack's **single** `etcd` service (`OPS_HA_ETCD_SERVICES=etcd`, a
6×lease-TTL convergence budget) and enforces the topology-aware tally in-harness
(`--expect-pass 10 --expect-skip 1`, an exit-code artifact) — **s01–s10 all PASS**,
including etcd-down and etcd-partition recovery, and `s11` (kill the etcd Raft leader
among a quorum) reports an **explicit topology SKIP** with its reason: single-node etcd
keeps no surviving quorum after a leader kill, so the resume expectation cannot apply.
Quorum-leader failover itself stays certified on the 3-node dev topology, where the
gate is `run-all --expect-pass 11 --expect-skip 0` — s11 always executes there, proving
the skip keys on topology, never a blanket. The offline chain joins nightly CI in W2 so
this path can't rot invisibly again.
...
./verify-egress-blocked.sh --target /opt/tess-engine   # egress self-check + split-brain step: 10 PASS + 1 topology-SKIP (s11 quorum-only), tally enforced in-harness
```
...
1. **Non-root containers — done.** The app image runs as `appuser` (uid 1000): user created
   ...
   `run-all` 11/11 on the non-root stack; offline chain green with non-root (the
   harness-step topology caveat found during those runs was fixed by W1.5 — see §Offline
   packaging).
2. **Reproducible rebuild — done.** `requirements.lock.txt` (hash-pinned, generated by
   `pip-compile --generate-hashes` in a linux/amd64 `python:3.11-slim` container,
   constrained to the verified image's `pip freeze` — lock≡freeze identity, zero
   ...
   Note the lock's own header records the original in-container command verbatim —
   including `--constraint=current-freeze.txt`, the container's transient `pip freeze`
   which is deliberately NOT in the repo — so the header is not runnable as written.
   The runnable regeneration procedure (reconstructs the same lock≡freeze identity):
   ...
   docker run --rm -v "$PWD":/w -w /w python:3.11-slim bash -c '
     ...
     pip-compile --constraint=current-freeze.txt --generate-hashes \
       --output-file=requirements.lock.txt --strip-extras requirements.txt &&
     rm current-freeze.txt'
   ```
